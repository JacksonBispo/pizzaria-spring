...
import br.com.smartpizza.dao.impl.produto.ProdutoDaoImpl;
...
public class BuscaProduto extends HttpServlet
{
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		switch(acao) {
		case "listar":
			List<ProdutoDTO> produtos = pdao.findByNameType(1L);
			
			String resp = new Gson().toJson(produtos);
			out.print(resp);
			break;
		case "listarBroto":
			List<ProdutoDTO> broto = pdao.findByNameTypeAndSize("pizza",1L,"broto");
		...
		List<ProdutoDTO> p = pdao.findByNameDTO(nome);
